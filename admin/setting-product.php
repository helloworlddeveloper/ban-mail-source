...
if($_POST && $_POST['type'] == 'them-loai-san-pham' && isset($_POST['value']) && isset($_POST['rate'])){
  ...
  if($check){
     ...
     mysqli_query($conn,"INSERT INTO `danh-sach-san-pham`(`key_id`, `value`, `rate`) VALUES ('$key_id','$value','$rate')");
  }
            ...
            <tbody>
                     <?php
                      ...
                      <tr>
                           ...
                           <td><?= $row['key_id']; ?></td>
                           <td><?= $row['value']; ?></td>
                           <td><?= number_format($row['rate']); ?> VNĐ</td>
     ...
     $(()=>{
         $('#them-loai-san-pham').click(()=>{
            ...
            if(!value){
                ...
                $.post('#',{type:'them-loai-san-pham',value,rate},'json')
                .done((res)=>{
